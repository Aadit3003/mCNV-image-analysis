...
#@ int (label = "Gaussian Blur Sigma", value = 1) gaussian_sigma
#@ int (label = "Mexican Hat Filter Radius", value = 13) mexican_hat_filter_radius
#@ int (label = "Number of Pixels per 1 mm", value = 170) scale_pixels_per_1_mm
#@ int (label = "Auto Local Threshold Radius", value = 8) auto_local_threshold_radius
...
function processFile(input, t_output, s_output, file) {
	...
	// SAVE
	saveStage(s_output, file, "_1_Delineated");
...
function mex_hat(){
	...
	run("Convert to Mask");
	// SAVE
    saveStage(s_output, file, "_3_Threshold");
}
function skeletonize(s_output, file){
	run("Skeletonize");
	// SAVE
    saveStage(s_output, file, "_4_Skeleton");
}
function gray(){
	run("8-bit");
...
function threshold(s_output, file){
	...
	// SAVE
	saveStage(s_output, file, "_2_Gaussian");
	...
	selectWindow("Results");
    run("8-bit");
    run("Auto Local Threshold", "method=Median radius="+auto_local_threshold_radius+" parameter_1=0 parameter_2=0 white");
...
function measureSkeleton(s_output, file){
    ...
    saveStage(s_output, file, "_5_Analyze_Skeleton"); // Tagged Skeleton
}
